...
class ClockPanel extends JPanel implements Runnable {
  ...
  private final static Color THE_COLOR = new Color(255,0,0);
  ...
  protected void paintComponent(Graphics g){
    ...
    // Draw the dots
    g2.setColor(THE_COLOR);
    for (int i = 0; i < 60; i++) {
      
      Point dotCoordinates = minToLocation(i, DISTANCE_DOT_FROM_ORIGIN);
      g2.setColor((i <= Calendar.getInstance().get(Calendar.SECOND)) ? Color.gray : THE_COLOR);
      
      if (i % 5 == 0) {
        // big dot
        g2.fillOval(
        		dotCoordinates.x - (DIAMETER_BIG_DOT / 2),
        ...
        // small dot
        g2.fillOval(
        		dotCoordinates.x - (DIAMETER_SMALL_DOT / 2),
